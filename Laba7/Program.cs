...
namespace Laba7
{
    ...
    class Product : Goods
    {
        public Product(double Price, string ProductName, int Weight)
        {
            ...
            if (ProductName == null) //!!!!!!!!!!!!!!!!!!!!!!
                throw new NameException("Введите название продукта!");
            else
                name = ProductName;
    ...
    class Flowers : Goods, Products
    {
        ...
        public Flowers(double Price, string FlowersName, int Numbers, int Weight)
        {
            ...
            if (price < 0)//!!!!!!!!!!!!!!!!!!!!!!
                throw new PriceException("Стоимость продукта не может быть отрицательной!");
            else
                price = Price;
        }
        public void Message()
    ...
    class Cake //переопределены все методы Object
    {
        ...
        public Cake(int Price, string CakeName, string Filling, int Weight)
        {
            ...
            if (weight < 1500)//!!!!!!!!!!!!!!!!!!!!!!
                throw new SaleException("Акция дествует при покупке торта от 1,5 кг");
            else
            {
                price = price / 2;
            }
